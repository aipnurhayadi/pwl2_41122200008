...
export default function Lecturers() {
    ...
    const run = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(err.detail ?? "Gagal memuat data dosen");
        }

        const body = await res.json();
        setAllRows(Array.isArray(body.lecturers) ? body.lecturers : []);
      } catch (e) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return allRows.filter((row) => {
      const haystack = `${row.code ?? ""} ${row.employee_code ?? ""} ${row.name ?? ""}`.toLowerCase();
    ...
    <main className="container mx-auto max-w-6xl px-4 py-8 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Mode baca saja sementara sampai endpoint assignment dosen Laravel dipindahkan.</p>
        </div>
      </div>
